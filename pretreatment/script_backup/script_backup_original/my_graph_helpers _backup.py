...
def form_equivalence_classes(myG, only_k_max=True,duals=None, verbose=False):
    print ("!!!!Start form_equivalence_classes!!!!!")
    ...
    while depth < len(duals):
        ...
        pass
    print ("!!!!Finish form_equivalence_classes!!!!!")
    return result, depth
...
def build_all_roads(myG,
    ...
    while (myG.interior_parcels and road_max):
        ...
        # flist from result!
            ...
            if md == 3:
                print ("form_equivalence_classes0")
                flist = myG.interior_parcels
            
            
            elif md > 3:
                if outsidein is False:
                    print ("form_equivalence_classes1")
                    result, depth = form_equivalence_classes(myG,only_k_max=random_road)
                    while len(flist) < 1:
                        md = max(result.keys())
                        print ("max(result.keys())",md)
                        print ("result",result)
                        flist = flist + result.pop(md)

                elif outsidein is True:
                    print ("form_equivalence_classes2")
                    # print ("myG",myG)
                    result, depth = form_equivalence_classes(myG,only_k_max=random_road)
                    md = max(result.keys())
                    #print ("md",md)
                    if len(result[md]) == 0:
                        md = md - 2
                    # print ("md",md)
                    # print (len(set(result[3])))
                    # print (len(set(result.get(5, []))))
                    flist = list(set(result[3]) - set(result.get(5, [])))
                    # print (len(flist))

            print ("md",md)
            print ("flist",len(flist),flist)
            fEdges = []
            for f in flist:
                for edge in f.edges:
